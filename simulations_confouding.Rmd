...
med_pred_sample <- 
  lapply(c(1000), function(sample_size){
    ...
    med_pred_sim <- 
      mclapply(c(1:20), mc.cores = 10, function(sim) {
        cf <- mvrnorm(n = sample_size,
                    mu = rep(0, 4),
                    Sigma = diag(4))
        cf5 <- sample(c((-2):2), sample_size, replace = T)
...
plot_data <- 
  ...
  mutate(model = factor(model, levels = c("linear_model", "linear_gps", "linear_gps_nocf", "gam_model", "gam_gps", "gam_gps_nocf")))  %>% 
...
pred_plot <-
  predictions %>%
  tidyr::pivot_longer(c("linear_model", "linear_gps", "gam_model", "gam_gps",  "true_fit")) %>%
   mutate(name = factor(name, levels = c("linear_model", "linear_gps", "gam_model", "gam_gps", "true_fit"))) %>% 
  data.table()
...
pred_summary <-
  ...
  mutate(name = relevel(name, ref = "linear_model")) %>% data.table()

# Get a sample of predictions to plot
pred_sample <- pred_plot[sim %in% sample(1:20, 10, replace = F)] 
...
ggplot() +
  geom_line(data = pred_summary[name != "true_fit" & sample_size == 1000 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = mean), linetype = "solid") +
   geom_line(data = true_fit_plot[sample_size == 1000 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = mean), color = "black", linetype = "dashed") +
    geom_line(data = pred_sample[name != "true_fit" & sample_size == 1000 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = value, color = factor(sim)), linetype = "solid", alpha = 0.5) + 
  scale_color_manual(values = c("gray", "gray","gray", "gray", "gray", "gray", "gray", "gray", "gray", "gray")) + 
  labs(x = "Exposure", y = "Response") +
  theme_bw() +
      ...
      mclapply(c(1:20), mc.cores = 10, function(sim) {
        ...
        med_pred_gps <- 
          lapply(c(1:6), function(gps_mod) {
            ...
            # Get metrics and predictions from sample
            metrics_predictions <- 
              metrics_from_data(exposure = exposure, confounders = confounders, relationship = "sublinear", 
                                sample_size = sample_size, family = "gaussian", adjust_confounder = T, trim = T)
...
med_pred_sample <- 
  lapply(c(200, 1000), function(sample_size){
    # Iterate through several simulations
    med_pred_sim <- 
      mclapply(c(1:100), mc.cores = 10, function(sim) {
        ...
        med_pred_gps <- 
          lapply(c(1:6), function(gps_mod) {
            ...
            return(list(metrics, predictions))
          })
        
        metrics_gps <- rbindlist(lapply(med_pred_gps, function(x) x[[1]]))
        predictions_gps <- rbindlist(lapply(med_pred_gps, function(x) x[[2]]))
        
        return(list(metrics_gps, predictions_gps))
      })
    metrics_sim <- rbindlist(lapply(med_pred_sim, function(x) x[[1]]))
    predictions_sim <- rbindlist(lapply(med_pred_sim, function(x) x[[2]]))
    return(list(metrics_sim, predictions_sim))
  })
...
plot_data %>% 
  filter(name == "bias", sample_size == 200) %>% 
  ggplot() + 
  geom_pointrange(aes(y = mean, ymin = lower, ymax = upper, x = factor(gps_mod), color = model), position = position_dodge(.5)) +
  geom_vline(xintercept=c(1.5, 2.5,3.5, 4.5, 5.5), linetype="dashed", size = 0.2) + 
  labs(x = "Scenario", y = "Value") + 
...
plot_data %>% 
  filter(name == "absolute_bias", sample_size == 200) %>% 
  ggplot() + 
  geom_pointrange(aes(y = mean, ymin = lower, ymax = upper, x = factor(gps_mod), color = model), position = position_dodge(.5)) +
  geom_vline(xintercept=c(1.5, 2.5,3.5, 4.5, 5.5), linetype="dashed", size = 0.2) + 
  labs(x = "Scenario", y = "Value") + 
...
plot_data %>% 
  filter(name == "mse", sample_size == 200) %>% 
  ggplot() + 
  geom_pointrange(aes(y = mean, ymin = lower, ymax = upper, x = factor(gps_mod), color = model), position = position_dodge(.5)) +
  geom_vline(xintercept=c(1.5, 2.5,3.5, 4.5, 5.5), linetype="dashed", size = 0.2) + 
  labs(x = "Scenario", y = "Value") + 
...
pred_plot <-
  predictions %>%
  tidyr::pivot_longer(c("linear_model", "gam_model", "weighted_gps", "true_fit")) %>%
  mutate(name = factor(name)) %>%
  data.table()
...
# Get a sample of predictions to plot
pred_sample <- pred_plot[sim %in% sample(1:100, 10, replace = F)] 
...
ggplot() +
  geom_line(data = pred_summary[name != "true_fit" & sample_size == 200 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = mean), linetype = "solid") +
   geom_line(data = true_fit_plot[sample_size == 200 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = mean), color = "black", linetype = "dashed") +
    geom_line(data = pred_sample[name != "true_fit" & sample_size == 200 & gps_mod %in% c(1, 2, 3)], aes(x = exposure, y = value, color = factor(sim)), linetype = "solid", alpha = 0.5) + 
  scale_color_manual(values = c("gray", "gray","gray", "gray", "gray", "gray", "gray", "gray", "gray", "gray")) + 
  labs(x = "Exposure", y = "Outcome") +
  theme_bw() +
...
ggplot() +
  geom_line(data = pred_summary[name != "true_fit" & sample_size == 200 & gps_mod %in% c(4, 5, 6)], aes(x = exposure, y = mean), linetype = "solid") +
   geom_line(data = true_fit_plot[sample_size == 200 & gps_mod %in% c(4,5,6)], aes(x = exposure, y = mean), color = "black", linetype = "dashed") +
    geom_line(data = pred_sample[name != "true_fit" & sample_size == 200 & gps_mod %in% c(4, 5, 6)], aes(x = exposure, y = value, color = factor(sim)), linetype = "solid", alpha = 0.5) + 
  scale_color_manual(values = c("gray", "gray","gray", "gray", "gray", "gray", "gray", "gray", "gray", "gray")) + 
  labs(x = "Exposure", y = "Outcome") +
  theme_bw() +
  coord_cartesian(ylim = c(0, 12)) + 
