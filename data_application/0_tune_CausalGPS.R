### File to test Tune causalGPS to achieve covariate balance
rm(list = ls())
...
# Create grid of hyperparmeters for xgboost and delta
tune_grid <- 
...
# Find minimum covariate balance and return psuedo pop
grid_min <- grid_search_corr %>% filter(corr == min(corr))
grid_min_results <- 
  fit_causalGPS_by_year(nrounds = grid_min$nrounds, 
                        max_depth = grid_min$depth,
                        eta = grid_min$eta, 
                        delta = grid_min$delta, 
                        return_corr_only = F, 
                        nthread = 10, 
                        trim = F)
...
# Now fit the Causal model
# Using the design weights only
causal_gps1 <- 
...
# Multiply design and strata weights
causal_gps2 <- 
  mgcv::bam(reformulate(c("s(pm25, bs = 'cr', k = 4)", strata_var, confounders, "year"), response = "log_mort"),
...
# # For looking at results (after run completed)
