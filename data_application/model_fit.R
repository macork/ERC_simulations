...
gam_fit <-
  mgcv::bam(log_mort ~ s(pm25_ensemble, bs = 'cs', k = 4) + medhouseholdincome + medianhousevalue + poverty + education + pct_owner_occ +as.factor(year) + as.factor(region) +
              as.factor(sex) + as.factor(race) + as.factor(dual) + as.factor(entry_age_break) + as.factor(followup_year),
            data = data)
...
data_ent <-
  data[, c("pm25_ensemble", "medhouseholdincome", "medianhousevalue", "poverty", "education",
                "pct_owner_occ", "year", "region", "sex", "race", "dual", "entry_age_break")]

# Create model matrx
data_ent_matrix <- model.matrix(pm25_ensemble ~ 0 + medhouseholdincome + medianhousevalue + poverty + education +
                                  pct_owner_occ + year + region + sex + race + dual + entry_age_break, data_ent)
...
e <- ebal(data_ent$pm25_ensemble, c_mat, base_weights = ent_weights)
ent_weights <- e$weights
ent_weights[ent_weights > quantile(ent_weights, 0.995)] <- quantile(ent_weights, 0.995)

# Now save entropy weights and covariate weights
cov.wt(cbind(data_ent$pm25_ensemble, c_mat), ent_weights, TRUE)$cor[, 1]

ent_dataset <- data.table(cbind(data_ent$pm25_ensemble, c_mat, ent_weights))
...
pseudo_pop <- 
  generate_pseudo_pop(data$log_mort,
                      ...
                      sl_lib = c("m_xgboost"),
                      params = list(xgb_nrounds = c(30, 50, 100, 200),
...
causal_gps <- 
  mgcv::bam(formula = Y ~ s(w, bs = 'cs', k = 4),
            family = "gaussian",
